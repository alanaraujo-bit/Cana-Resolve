...
export default async function PartnerNotificationsPage() {
  ...
  const items = await partnerNotifications(viewer.id);
  return <AudienceShell audience="partner" title="Atualizações"><p className="text-brand-ink text-xs font-bold tracking-[0.14em] uppercase">Atualizações</p><h2 className="mt-2 text-3xl">O que mudou por aqui.</h2>{items.length ? <div className="mt-7 space-y-3">{items.map((item) => <a key={item.id} href={item.href || "#"} className="border-line bg-surface block rounded-2xl border p-4"><p className="font-semibold">{item.title}</p><p className="text-muted mt-1 text-sm">{item.body}</p></a>)}</div> : <div className="border-line bg-surface mt-7 rounded-2xl border p-6"><p className="font-semibold">Sem novas atualizações.</p><p className="text-muted mt-1 text-sm">Novas oportunidades e informações importantes aparecerão aqui.</p></div>}</AudienceShell>;
}
